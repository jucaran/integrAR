export default {
  ...
  Query: {
    ...
    students: async (parent, { order_by, attribute }, ctx) => {
      if (order_by && attribute)
        return await ctx.Student.find().sort({ [attribute]: order_by });
      else return await ctx.Student.find();
    },
  ...
  Mutation: {
    ...
    editSuperAdmin: async (_, args, ctx) => {
      return await ctx.SuperAdmin.findByIdAndUpdate(args._id, args.input, {
        new: true,
      });
    },
    ...
    editTeacher: async (_, args, ctx) => {
      return await ctx.Teacher.findByIdAndUpdate(args._id, args.input, {
        new: true,
      });
    },
    ...
    editStudent: async (_, args, ctx) => {
      return await ctx.Student.findByIdAndUpdate(args._id, args.input, {
        new: true,
      });
    },
    ...
    editCourse: async (_, args, ctx) => {
      return await ctx.Course.findByIdAndUpdate(args._id, args.input, {
        new: true,
      });
    },
    ...
    editSubject: async (_, args, ctx) => {
      return await ctx.Subject.findByIdAndUpdate(args._id, args.input, {
        new: true,
      });
    },
